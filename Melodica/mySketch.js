...
=======================================================================
Description:
"Melodica" is a script designed to create serene and soothing meditation melodies. 
Crafted with a passion for programming and a love for harmonious sounds, 
Melodica offers a unique blend of functionality and artistry. 
Through carefully curated algorithms, this script generates melodies that are 
tailored to induce a sense of calmness and tranquility, providing an ideal backdrop 
for meditation sessions.

The script incorporates a variety of musical elements, such as gentle chord 
progressions, ambient tones, and carefully timed pauses, all meticulously 
orchestrated to guide the listener into a state of relaxation. 
The programming prowess behind Melodica ensures that each melody is 
not just a random arrangement of notes but a thoughtful composition aimed at 
promoting mindfulness and inner peace.

Users can customize their meditation experience by adjusting parameters like tempo, 
instrumentation, and the overall mood of the generated melodies. Whether you prefer a 
soft piano melody, ethereal synth tones, or a combination of instruments, 
Melodica allows for a personalized meditation soundtrack to suit individual preferences.

Melodica is more than just a script; it's a creative tool that harnesses 
the beauty of programming to enhance the meditative journey. With its 
elegant design and focus on creating harmonious atmospheres, 
Melodica stands as a testament to the symbiosis of technology 
...
=======================================================================
*/
let tokenData = {"hash":"0x20649fbae350e8aaff7c82482d732c416ccede0918d7f2476ad42ea20c9f4770","tokenId":"140000708"}
...
let features,Fcolor,FMelodyNum,FframeStyle,FframeColor,Fshape,FrectNum,Fsine,FTraingle,FSquare,FSaw,synthId,FbgShape,FspecialLine,ring,ringX,ringY,frameOutline=0,
    ...
    frqArray=[130.82,146.83,164.81,174.61,196,220,246.94,261.6,293.7,329.6,349.2,392,440,493.9,523.25,587.33,659.26,698.46,783.99,880,987.77],noteLength=[.8,1.2,1.6,2],mSize=[2,2,2,2,4,4,4,10,4,4,4,4,10,4,4,4,6,6,16,6,6,6,6,6,6,6,6,10,10,10,16,16],grad=[["#686868","#999999","#444444","#B5B5B4","#D3D3D3"],["#FCE355","#FFCC3B","#6D3503","#B28E36","#572600"],["#232323","#606060","#3D3D3D","#B5B5B4","#3D3D3D"],["#20326E","#62E383","#8083E9","#3B5CCB","#C23447"],["#3B5F88","#2D4F37","#F3BD45","#845F4E","#CB5958"]];
...
function draw() {
  ...
  for (let i = 0; i < MelodyNum; i++)if (melodys[i].later == 3) melodys[i].showBg();
  for (let i = 0; i < MelodyNum; i++)if (melodys[i].later == 2) melodys[i].showBg();
  for (let i = 0; i < MelodyNum; i++)if (melodys[i].later == 1) melodys[i].showBg();
  for (let i = 0; i < MelodyNum; i++)if (melodys[i].later == 0) melodys[i].showBg();
  for (let i = 0; i < totalLine; i++) if (lines[i] != null) lines[i].display();
  for (let i = 0; i < MelodyNum; i++) melodys[i].run();
  drawframe();
  if (metaCount < 314) recordMeta(), showMeta();
  if(frameCount%20==0){
  //   seed = int(random(500000));
  // play(1, 1000, 0.1, 0.2);
  // compose();
  // soundGo = 0;
  // cctt++;
  // save(cctt+"_"+seed+".png");
...
class Melody {
  ...
  showLine() {
    ...
    this.tx =
      this.circleX + (cos(this.angle + this.arcStart) * this.circleRadius) / 2;
    this.ty =
      this.circleY + (sin(this.angle + this.arcStart) * this.circleRadius) / 2;
...
function defult() {
  ...
  for (let i = 0; i < MelodyNum; i++) {
    ...
    for (let j = 0; j < linesNum; j++) {
      ...
      if (lineDir == 0) {
        x =
          average2(melodys[i].circleRadius, lineDist, j) * xInvert +
          melodys[i].circleX;
        y =
          average2(melodys[i].circleRadius, lineDist, 0) * yInvert +
          melodys[i].circleY;
      } else if (lineDir == 1) {
        x =
          average2(melodys[i].circleRadius, lineDist, 0) * xInvert +
          melodys[i].circleX;
        y =
          average2(melodys[i].circleRadius, lineDist, j) * yInvert +
          melodys[i].circleY;
      } else if (lineDir == 2) {
        x =
          average2(melodys[i].circleRadius, lineDist, j) * xInvert +
...
function recordMeta() {
  ...
  if (metaCount <= 314) {
    ...
    if (metaCount == 314) {
      console.log("done");
      save(meta, tokenData.tokenId+".png");
    }
  }
}